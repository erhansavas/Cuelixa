...
This document describes the Cuelixa 0.7.3 implementation. It is a source-of-truth overview, not an aspirational design document.
...
The public build is ad-hoc signed because the project does not currently use a paid Developer ID distribution identity. That provides a code-signing seal but not Apple developer authentication or notarization, so the release documentation states those limits explicitly. The release pipeline uses exact-tree manifests, exact-version notes, checksum verification, immutable GitHub releases, read-only post-publication verification, and both hosted and independent local qualification to detect different classes of build/release drift. GitHub's immutable-release attestation binds the published tag, commit, and assets; it is not treated as build provenance for the locally built DMG. See [Release procedure](RELEASE.md).
...
Cuelixa 0.7.3 targets **Apple silicon (`arm64`) and macOS 27.0 or later**. The release was qualified while macOS 27 and Xcode 27 were in Release Candidate phase, so the GitHub distribution is a pre-release.
